...
function AdminUsers() {
  ...
  const saveUser = async () => {
    ...
    const userData: Omit<AdminUser, "id" | "authUserId"> = {
      name: name.trim(),
  ...
  const sendPasswordReset = async (
    ...
    if (!user.email.trim()) {
      setResetMessage(
        "Uporabnik nima vpisanega e-naslova."
      );
      return;
    }
    ...
    try {
      ...
      if (error) {
        setResetMessage(
          `E-pošte ni bilo mogoče poslati: ${error.message}`
        );
        return;
      }

      setResetMessage(
        `Povezava za spremembo gesla je bila poslana na ${user.email}.`
      );
    } catch (error) {
      console.error(
        "Napaka pri pošiljanju povezave za spremembo gesla:",
        error
      );
      setResetMessage(
        "Povezave za spremembo gesla ni bilo mogoče poslati."
      );
    } finally {
    ...
    <div>
      ...
      {showForm && (
        ...
        >
          ...
          >
            Začetno geslo uporabi uporabnik za prvo prijavo.
            Supabase ID se ustvari samodejno in ga administrator
            ne vpisuje. Uporabnik lahko geslo pozneje spremeni v
            svojih nastavitvah.
          </div>
