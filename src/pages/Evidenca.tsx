...
function Evidenca() {
  ...
  /* =========================================
     FILTRIRANJE PO OBDOBJU IZ HEADERJA
  ========================================= */
  ...
  const filteredWorkOrders =
    useMemo(() => {
      return workOrders.filter(
        (workOrder) =>
          workOrder.date >= startDate &&
          workOrder.date <= endDate
      );
    }, [
      workOrders,
      startDate,
      ...
      return Object.entries(days)
        .map(
          ([date, values]) => ({
            ...
            regular: Number(
              values.regular.toFixed(2)
            ),

            night: Number(
              values.night.toFixed(2)
            ),

            holiday: Number(
              values.holiday.toFixed(2)
            ),

            additional: Number(
              values.additional.toFixed(2)
            ),
  ...
  return (
    <div>
      ...
      >
        <SummaryCard
          title="Redne ure"
          value={`${regularHours.toFixed(2)} h`}
          color="#2563eb"
        />

        <SummaryCard
          title="Nočne ure"
          value={`${nightHours.toFixed(2)} h`}
          color="#7c3aed"
        />

        <SummaryCard
          title="Nedelje / prazniki"
          value={`${holidayHours.toFixed(2)} h`}
          color="#dc2626"
        />

        <SummaryCard
          title="Dodatne ure"
          value={`${additionalHours.toFixed(2)} h`}
          color="#f97316"
        />
      ...
      >
        ...
        {dailyData.length === 0 ? (
          ...
          >
            Za izbrano obdobje ni
            vpisanih delovnih nalogov.
          </div>
        ) : (
          <div
            style={{
                ...
                {dailyData.map(
                  (day) => (
                    <tr key={day.date}>
                      ...
                      <td style={tableCellStyle}>
                        {day.regular.toFixed(2)} h
                      </td>

                      <td style={tableCellStyle}>
                        {day.night.toFixed(2)} h
                      </td>

                      <td style={tableCellStyle}>
                        {day.holiday.toFixed(2)} h
                      </td>

                      <td style={tableCellStyle}>
                        {day.additional.toFixed(2)} h
                      </td>
