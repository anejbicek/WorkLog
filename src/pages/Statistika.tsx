...
function Statistika() {
  ...
  const uniqueBreakdown = calculateUniqueHourBreakdown(monthWorkOrders);

  const regularHours = uniqueBreakdown.regular;
  const nightHours = uniqueBreakdown.night;
  const holidayHours = uniqueBreakdown.holiday;
  const overtimeHours = uniqueBreakdown.overtime;
      ...
      () => {
        ...
        monthWorkOrders.forEach(
          (
            ...
            // V dnevnem prikazu ne seštevamo kartic, ampak združimo njihove intervale.
            days[
              workOrder.date
            ].hours =
              calculateDailyUniqueWorkHours(
            ...
            if (
              workOrder.additionalMachine ||
              days[workOrder.date].additionalHours > 0
            ) {
              days[
                workOrder.date
              ].twoMachines =
    ...
    >
      ...
      {editingWorkOrder && (
        <WorkOrderEditModal
          ...
          workOrders={editingWorkOrders}
          onSave={(updatedWorkOrder) => {
            void updateWorkOrder(updatedWorkOrder);
            setEditingWorkOrder(null);
            setEditingWorkOrders([]);
          }}
          onSaveMany={async (updatedWorkOrders) => {
            await Promise.all(
              updatedWorkOrders.map((order) => updateWorkOrder(order))
            );
            setEditingWorkOrder(null);
            setEditingWorkOrders([]);
          }}
